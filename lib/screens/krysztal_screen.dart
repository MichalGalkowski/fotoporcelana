import 'package:flutter/material.dart';
import 'package:fotoporcelana/helpers/attachments_db.dart';
import 'package:fotoporcelana/helpers/products_db.dart';
import 'package:fotoporcelana/models/attachments.dart';
import 'package:fotoporcelana/models/product.dart';
import 'package:fotoporcelana/my_colors.dart';
import 'package:fotoporcelana/providers/data_provider.dart';
import 'package:fotoporcelana/providers/krysztal_options.dart';
import 'package:fotoporcelana/widgets/krysztal/krysztal_settings.dart';
import 'package:fotoporcelana/widgets/krysztal/schemat.dart';
import 'package:provider/provider.dart';

class KrysztalScreen extends StatefulWidget {
  static const routeName = '/krysztal';
  const KrysztalScreen({super.key});

  @override
  State<KrysztalScreen> createState() => _KrysztalScreenState();
}

class _KrysztalScreenState extends State<KrysztalScreen> {
  final title = 'Dodaj kryształ';

  void _incrementIndex(BuildContext context) {
    Provider.of<KrysztalOptions>(context, listen: false).incrementIndex();
  }

  void _decrementIndex(BuildContext context) {
    Provider.of<KrysztalOptions>(context, listen: false).decrementIndex();
  }

  void _resetKrysztal(BuildContext context) {
    Provider.of<KrysztalOptions>(context, listen: false).resetKrysztal();
  }

  @override
  Widget build(BuildContext context) {
    var krysztalProvider = Provider.of<KrysztalOptions>(context);
    var index = krysztalProvider.getIndex;
    var ksztalt = krysztalProvider.getKsztalt;
    var rozmiar = krysztalProvider.getRozmiar;
    var kolor = krysztalProvider.getKolor;
    var tlo = krysztalProvider.getTlo;
    var ubranie = krysztalProvider.getUbranie;
    var dwieOsoby = krysztalProvider.getDwieOsoby;
    var odbicie = krysztalProvider.getOdbicie;
    var kolorowanie = krysztalProvider.getKolorowanie;
    var express = krysztalProvider.getExpress;
    var wizualka = krysztalProvider.getWizualka;
    var message = krysztalProvider.getMessage;
    var amount = krysztalProvider.getAmount;

    var imageFilePaths =
        Provider.of<KrysztalOptions>(context, listen: false).getFilePaths;
    var pathNames = '';
    for (var element in imageFilePaths) {
      pathNames = '$pathNames - ${element.substring(62)}';
    }

    var title =
        'Kryształ ${ksztalt.displayName} ${rozmiar.displayName}, ${kolor.displayName}';
    var data = '-Tło: ${tlo.displayName} \n'
        '-Ubranie: ${ubranie.displayName} \n'
        '${dwieOsoby ? '-Dwie osoby na zdjęciu\n' : ''}'
        '${odbicie && dwieOsoby ? '-Mężczyzna z lewej\n' : ''}'
        '${odbicie ? '-Odbicie lustrzane\n' : ''}'
        '${kolorowanie ? '-Kolorowanie zdjęcia\n' : ''}'
        '${express ? '-Tryb express\n' : ''}'
        '${wizualka ? '-Wizualizacja\n' : ''}'
        '${message.isEmpty ? '' : message}';

    Future addProduct(String id) async {
      final product = Product(
        id: id,
        title: title,
        data: data,
        attachments: pathNames,
        amount: amount,
        createdTime: DateTime.now(),
      );
      await ProductsDB.instance.create(product);
    }

    Future addAttachments(String productID) async {
      Attachments attachment;
      for (String item in imageFilePaths) {
        attachment = Attachments(
          path: item,
          product: productID,
          createdTime: DateTime.now(),
        );
        await AttachmentsDB.instance.create(attachment);
      }
    }

    return Scaffold(
      backgroundColor: Colors.grey[200],
      resizeToAvoidBottomInset: false,
      bottomNavigationBar: index != 0
          ? BottomNavigationBar(
              selectedItemColor: MyColors.mainMaterial[50],
              unselectedItemColor: MyColors.mainMaterial[50],
              backgroundColor: MyColors.accentMaterial,
              onTap: (value) {
                if (value == 2) {
                  if (index == 7) {
                    String dateTimeNow = DateTime.now().toString();
                    addProduct(dateTimeNow);
                    addAttachments(dateTimeNow);
                    _resetKrysztal(context);
                    Provider.of<DataProvider>(context, listen: false)
                        .changeIndex(2);
                    Provider.of<DataProvider>(context, listen: false)
                        .incrementCartItems();
                    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
                        content: Text('Kryształ dodany do koszyka')));
                    Navigator.of(context).pop();
                  } else {
                    _incrementIndex(context);
                  }
                } else if (value == 1) {
                  _resetKrysztal(context);
                  Navigator.of(context).pop();
                } else {
                  _decrementIndex(context);
                }
              },
              items: [
                  const BottomNavigationBarItem(
                    icon: Icon(Icons.arrow_back),
                    label: 'Poprzednie',
                  ),
                  const BottomNavigationBarItem(
                    icon: Icon(Icons.cancel),
                    label: 'Anuluj',
                  ),
                  BottomNavigationBarItem(
                    icon: index == 7
                        ? const Icon(Icons.add_shopping_cart)
                        : const Icon(Icons.arrow_forward),
                    label: index == 7 ? 'Dodaj do koszyka' : 'Następne',
                  ),
                ])
          : BottomNavigationBar(
              selectedItemColor: MyColors.mainMaterial[50],
              unselectedItemColor: MyColors.mainMaterial[50],
              backgroundColor: MyColors.accentMaterial,
              onTap: (value) {
                if (value == 1) {
                  _incrementIndex(context);
                } else {
                  if (index != 0) {
                    _decrementIndex(context);
                  } else {
                    _resetKrysztal(context);
                    Navigator.of(context).pop();
                  }
                }
              },
              items: const [
                  BottomNavigationBarItem(
                    icon: Icon(Icons.cancel),
                    label: 'Anuluj',
                  ),
                  BottomNavigationBarItem(
                    icon: Icon(Icons.arrow_forward),
                    label: 'Następne',
                  ),
                ]),
      body: SafeArea(
        child: Column(mainAxisAlignment: MainAxisAlignment.start, children: [
          Container(
            color: MyColors.mainMaterial,
            child: Padding(
              padding: const EdgeInsets.symmetric(vertical: 10.0),
              child: Center(
                  child: Text(
                title,
                overflow: TextOverflow.ellipsis,
                style: const TextStyle(color: Colors.white),
              )),
            ),
          ),
          if (index != 6 && index != 7) ...[
            const SizedBox(
              height: 10.0,
            ),
            const Expanded(child: Schemat()),
          ],
          const SizedBox(
            height: 10.0,
          ),
          const KrysztalSettings(),
        ]),
      ),
    );
  }
}
